...
void CMPAFile::CalcBytesPerSec()
{
	...
	{
		std::uint32_t dwBytes = m_pVBRHeader->m_dwBytes;
...
CMPAFrame *CMPAFile::GetFrame(CMPAFile::GetType Type, CMPAFrame *pFrame, bool bDeleteOldFrame, std::uint32_t dwOffset)
{
	...
	catch (CMPAException &e)
	{
		...
		// No direct access to debugger, we print to stderr instead.
		std::fprintf(stderr, e.GetErrorDescription());
		pNewMPAFrame = NULL;
	}
