...
CMPAFileStream::CMPAFileStream(LPCTSTR szFilename) : CMPAStream(szFilename), m_dwOffset(0)
{
...
CMPAFileStream::CMPAFileStream(LPCTSTR szFilename, HANDLE hFile) : CMPAStream(szFilename), m_hFile(hFile)
{
	Init();
}
...
void CMPAFileStream::SetPosition(DWORD dwOffset) const
{
	...
	if (result == INVALID_SET_FILE_POINTER && GetLastError() != NO_ERROR)
	{
		// != NO_ERROR
		// throw error
		throw CMPAException(CMPAException::ErrSetPosition, m_szFile, "SetFilePointer", true);
	}
}
...
DWORD CMPAFileStream::GetSize() const
{
	...
	if (dwSize == INVALID_FILE_SIZE)
		throw CMPAException(CMPAException::ErrReadFile, m_szFile, "GetFileSize", true);
	return dwSize;
}
