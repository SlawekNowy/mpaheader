...
const std::uint32_t CMPAFrame::m_dwProtectedBitsLayer2[5][2] =
	{
		{284, 142}, // table a
		{308, 154}, // table b
		{42, 84},	// table c
...
CMPAFrame::CMPAFrame(CMPAStream *pStream, std::uint32_t &dwOffset, bool bFindSubsequentFrame, bool bExactOffset, bool bReverse, CMPAHeader *pCompareHeader) : m_pStream(pStream),
																																							  m_dwOffset(dwOffset),
																																							  m_bIsLast(false)
{
	...
	if (bFindSubsequentFrame)
	{
		...
		std::uint32_t dwNewOffset = GetSubsequentHeaderOffset();
		try
		{
		...
		*/
		...
		{
			std::fprintf(stderr, "Didn't find subsequent frame");
...
bool CMPAFrame::CheckCRC() const
{
	...
	std::uint32_t dwByteSize = (std::uint32_t)ceil(dwProtectedBits / 8.0);
