...
CXINGHeader *CXINGHeader::FindHeader(const CMPAFrame *pFrame)
{
	...
	std::uint32_t dwOffset = pFrame->m_dwOffset + MPA_HEADER_SIZE + pFrame->m_pHeader->GetSideInfoSize(); // + (pFrame->m_pHeader->m_bCRC?2:0);
...
CXINGHeader::CXINGHeader(const CMPAFrame *pFrame, std::uint32_t dwOffset) : CVBRHeader(pFrame->m_pStream, dwOffset)
{
	/* XING VBR-Header
	...
	*/
	...
	if (dwFlags & TOC_FLAG)
	{
		...
		if (m_pnToc)
		{
			for (std::uint32_t i = 0; i < m_dwTableSize; i++)
...
std::uint32_t CXINGHeader::SeekPosition(float &fPercent) const
{
